...
-(void)buildViews{
    ...
    [_phoneInput setTextLabel:@"账       号" withSeparator:@":"];
...
//    [_registerBtn addTarget:self action:@selector(didTapRegisterBtn:) forControlEvents:UIControlEventTouchUpInside];
    ...
    _registerBtn = [[ZTRoundButton alloc] initWithTitle:@"下一步" textcolor:[UIColor whiteColor]
                                        backgroundImage:[UIImage imageNamed:@"btn_bg_nomal"]];
    
     [_registerBtn addTarget:self action:@selector(nextFoot:) forControlEvents:UIControlEventTouchUpInside];
...
-(void)requestUrl:(NSString *)request processParamsInBackground:(NSMutableDictionary *)params{
    ...
    }else if ([request isEqualToString:[NSString stringURLWithAppendex:NET_API_VERYZM]]){
        [params addEntriesFromDictionary:@{
                                           NET_ARG_VERYZM_MOBILE:[self getPhone],
                                           NET_ARG__VERYZM_YZM:[self getYzm],
                                           }];
...
-(void)requestUrl:(NSString *)request resultSuccess:(id)result{
    ...
    if ([request isEqualToString:[NSString stringURLWithAppendex:NET_API_GETYZM]]) {
        ...
        if ([_yzm validate]) {
            [[ZTNetWorkUtilities sharedInstance] doPost:[NSString stringURLWithAppendex:NET_API_VERYZM] delegate:self cancelIfExist:YES];
        ...
        vc.username=[self getPhone];
        vc.mobyzm=[self getYzm];
        vc.taggg=self.tttag;
