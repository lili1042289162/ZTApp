...
+(id)initWithIcon:(UIImage*)icon title:(NSString*)title indicator:(BOOL)showIdicator andendtitle:(NSString*)endtitle{
    SaleBarItem *item = [[SaleBarItem alloc] init];
    item.icon = icon;
    [item setTitle:title forState:UIControlStateNormal];
   item.endtitle.text=endtitle;
   
    [[[item.endtitle fitSize]setRectCenterVertical]setRectOnRightSideOfView:item.titleLabel];
