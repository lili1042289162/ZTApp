 ...
 */
-(void)initTitleBar{
    ZTTitleLabel *titleView = [[ZTTitleLabel alloc] initWithTitle:@"中团超市"];
...
-(void)buildHeaderViewtwo{
   _cateGoryButton = [[ZTButtonGridView alloc] initWithIcons:_category_source cell:@"ZTButtonCell" column:4 rowSpace:0.0 columnSpace:0.0 edgeSpace:0.0 version:nil];
...
- (UICollectionReusableView *)collectionView:(UICollectionView *)collectionView viewForSupplementaryElementOfKind:(NSString *)kind atIndexPath:(NSIndexPath *)indexPath{
    ...
    [[[_cateGoryButton fitSize] setRectBelowOfView:scrollview]addRectY:5.0];
...
NSLog(@"fanhuitemaishangpingshuju%@",result);
 ...
 [_TM_ProductList addObjectsFromArray:[[ZTDataCenter sharedInstance] getProductsFromPage:1 pageSize:10 offSet:0 count:10 firstorderBy:@"sdate" asic:NO secondorderBy:@"tmdj" asic:NO
 thirdorderBy:nil asic:nil type:CKEY_TE_MAI ]];
       ...
       // [_TM_ProductList addObjectsFromArray:[[ZTDataCenter sharedInstance] getProductsFromPage:1 pageSize:10 offSet:0 count:10 orderBy:@"tmid" asic:NO type:CKEY_TE_MAI]];
        ...
        for (int i=0; i<_TM_ProductList.count; i++) {
            TeMaiProduct*a=_TM_ProductList[i];
            NSLog(@"2aaaaqq%@",a.tmdj);
            NSLog(@"1aaaaqq%@",a.sdate);
        }
        NSLog(@"save%@save",tm_products);

     
 NSLog(@"kongma%@",_TM_ProductList);
    }
}
