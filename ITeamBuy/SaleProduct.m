...
-(void)initBootomBar{
    ...
    [_collectionBtn addTarget:self action:@selector(didTapCollectionBtnn:) forControlEvents:UIControlEventTouchUpInside];
...
-(void)didTapCollectionBtnn:(UIButton*)btn{
NSString *token = [[NSUserDefaults standardUserDefaults] objectForKey:UD_KEY_CURRENT_TOKEN];
    if (nil==token) {
        ...
        {
            [[ZTNetWorkUtilities sharedInstance] doPost:[NSString stringURLWithAppendex:NET_API_ADDCART] delegate:self cancelIfExist:YES ];
            [ZTCoverView alertCover];}
NSLog(@"collect");
    }
    }
...
NSLog(@"oko");
    ...
    if ([request isEqualToString:[NSString stringURLWithAppendex:NET_API_ADDCART]]){
        NSString*mess=@"成功加入购物车";
        alertShow(mess);
...
-(void)requestUrl:(NSString*)request processParamsInBackground:(NSMutableDictionary*)params{
    ...
    if ([request isEqualToString:[NSString stringURLWithAppendex:NET_API_ADDCART]]) {
        [params addEntriesFromDictionary:@{
                                           NET_ARG_ADDCART_TMID:[NSString stringWithFormat:@"%@",self.pid],
                                          NET_ARG_ADDCART_TMSL:@"1",
                                          NET_ARG_ADDCART_BTIMES:[NSString stringWithFormat:@"%@",_product.buytimes],
                                          NET_ARG_ADDCART_BNUMS:[NSString stringWithFormat:@"%@",_product.buynums],
                                           NET_ARG_ADDCART_CMID:[NSString stringWithFormat:@"%@",selectcima.tmcid],
