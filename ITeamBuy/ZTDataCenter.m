...
-(void)saveToDisk{
    ...
    NSLog(@"cuncuncucnuc%@",cacheDirectory);
...
-(NSArray*)getProductsFromPage:(NSInteger)page
                      pageSize:(NSInteger)size
                        offSet:(NSInteger)offset
                         count:(NSInteger)count
                       firstorderBy:(NSString*)at
                          asic:(BOOL)asic
                  secondorderBy:(NSString*)att
                          asic:(BOOL)asicc
                 thirdorderBy:(NSString*)attt
                          asic:(BOOL)asiccc
                          type:(NSString*)type
{
    NSArray *products = [[_cache objectForKey:type] allValues];
    ...
    NSSortDescriptor *firstsort = [NSSortDescriptor sortDescriptorWithKey:at ascending:asic];
    // 2.再按照对象第二个属性进行排序
    NSSortDescriptor *seclondsort = [NSSortDescriptor sortDescriptorWithKey:att ascending:asicc];
    // 3.再按照对象第三个属性进行排序
   //NSSortDescriptor *thirdsort = [NSSortDescriptor sortDescriptorWithKey:attt ascending:asiccc];
    // 按顺序添加排序描述器
    NSArray *descs = [NSArray arrayWithObjects:firstsort, seclondsort,nil];
    
    NSArray *sorteAry = [products sortedArrayUsingDescriptors:descs];
