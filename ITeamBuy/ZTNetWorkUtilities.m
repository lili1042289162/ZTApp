...
-(instancetype)init{
    if (self = [super init]) {
        _taskPool = [[NSMutableDictionary alloc] initWithCapacity:10];
    }
    return self;
...
//创建一个串行执行队列
    ...
    dispatch_async(taskToken, ^{
        ...
        NSLog(@"wangluqingqiufanhuishuju%@",resultData);
        //对返回数据解析
...
-(void)url:(NSString*)stringUrl delegate:(id<NetResultProtocol>)delegate JsonDataParse:(NSData*)result{
    ...
    NSLog(@"xitongzidaijson%@",resultDic);
    if (nil == resultDic) {
        // 外层数据解释失败
